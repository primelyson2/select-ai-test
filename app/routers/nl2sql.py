...
# ── 질문관리(관리 화면) 전용: 유사질문 테이블 + 질문/유사질문 임베딩 벡터 ─────
# 벡터는 서버 SQL 안에서만 생성·저장한다(DBMS_VECTOR.UTL_TO_EMBEDDING 인라인).
# Python 으로 VECTOR 를 주고받지 않으므로 thin-mode 왕복 문제가 없다. 23ai 기능.
async def _ensure_admin_tables(database: str) -> None:
    """기본 질문/컬럼 테이블 + 유사질문 테이블(+질문 벡터 컬럼) 멱등 보장."""
    await _ensure_tables(database)
    await db.execute(
        database,
        "CREATE TABLE IF NOT EXISTS T_NL2SQL_SIMILAR ("
        " id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,"
        " question_id NUMBER, similar_question CLOB, similar_vector VECTOR,"
        " created_at TIMESTAMP DEFAULT SYSTIMESTAMP)",
    )
    # QUESTION_VECTOR 멱등 추가(이미 있으면 ORA-01430 무시).
    await db.execute(
        database,
...
async def _q_insert(database: str, question: str, embed_params: str) -> tuple[int, bool]:
    """질문 INSERT. embed_params 있으면 question_vector 인라인 임베딩, 실패 시 텍스트만."""
    if embed_params:
        try:
            new_id = await _insert_returning_id(
                database,
                "INSERT INTO T_NL2SQL_QUESTION (question, question_vector) VALUES "
                "(:q, DBMS_VECTOR.UTL_TO_EMBEDDING(TO_CLOB(:q), JSON(:params))) "
                "RETURNING id INTO :id",
                q=question, params=embed_params,
            )
            return new_id, True
        except Exception:  # noqa: BLE001 — 임베딩 실패 → 텍스트만 저장
            pass
    new_id = await _insert_returning_id(
        database,
...
async def _q_update(database: str, qid: int, question: str, embed_params: str) -> bool:
    """질문 텍스트 수정(+embed_params 시 벡터 재생성). 반환: 임베딩 성공 여부."""
    if embed_params:
        try:
            await db.execute(
                database,
                "UPDATE T_NL2SQL_QUESTION SET question = :q, "
                "question_vector = DBMS_VECTOR.UTL_TO_EMBEDDING(TO_CLOB(:q), JSON(:params)) "
                "WHERE id = :qid",
                q=question, params=embed_params, qid=qid,
            )
            return True
        except Exception:  # noqa: BLE001
            pass
    await db.execute(
        database, "UPDATE T_NL2SQL_QUESTION SET question = :q WHERE id = :qid",
        q=question, qid=qid,
    )
    return False


async def _s_insert(database: str, question_id, text: str, embed_params: str) -> tuple[int, bool]:
    """유사질문 INSERT(+embed_params 시 similar_vector 임베딩, 실패 시 텍스트만)."""
    if embed_params:
        try:
            new_id = await _insert_returning_id(
                database,
                "INSERT INTO T_NL2SQL_SIMILAR (question_id, similar_question, similar_vector) VALUES "
                "(:qid, :t, DBMS_VECTOR.UTL_TO_EMBEDDING(TO_CLOB(:t), JSON(:params))) "
                "RETURNING id INTO :id",
                qid=question_id, t=text, params=embed_params,
            )
            return new_id, True
        except Exception:  # noqa: BLE001
            pass
    new_id = await _insert_returning_id(
        database,
        "INSERT INTO T_NL2SQL_SIMILAR (question_id, similar_question) "
        "VALUES (:qid, :t) RETURNING id INTO :id",
        qid=question_id, t=text,
    )
    return new_id, False


async def _s_update(database: str, sid: int, text: str, embed_params: str) -> bool:
    """유사질문 수정(+embed_params 시 벡터 재생성). 반환: 임베딩 성공 여부."""
    if embed_params:
        try:
            await db.execute(
                database,
                "UPDATE T_NL2SQL_SIMILAR SET similar_question = :t, "
                "similar_vector = DBMS_VECTOR.UTL_TO_EMBEDDING(TO_CLOB(:t), JSON(:params)) "
                "WHERE id = :sid",
                t=text, params=embed_params, sid=sid,
            )
            return True
        except Exception:  # noqa: BLE001
            pass
    await db.execute(
        database, "UPDATE T_NL2SQL_SIMILAR SET similar_question = :t WHERE id = :sid",
        t=text, sid=sid,
    )
    return False
...
async def add_question(payload: dict, database: str = Depends(current_db)) -> dict:
    """질문 등록. embed_params(JSON) 가 오면 question_vector 를 임베딩(질문관리 화면).
    없으면 기존 동작(Table list 팝업 호환)."""
    question = (payload.get("question") or "").strip()
    embed_params = (payload.get("embed_params") or "").strip()
    if not question:
        raise HTTPException(status_code=400, detail={"error": "질문을 입력하세요"})
    if embed_params:
        await _ensure_admin_tables(database)
    else:
        await _ensure_tables(database)
    new_id, embedded = await _q_insert(database, question, embed_params)
    warning = "embed_failed" if (embed_params and not embedded) else None
    return {"id": new_id, "question": question, "warning": warning}
...
async def delete_question(qid: int, database: str = Depends(current_db)) -> dict:
    """질문 삭제 — 연결된 조회컬럼·유사질문을 먼저 삭제한 뒤 질문을 삭제한다."""
    await _ensure_tables(database)
    await db.execute(database, "DELETE FROM T_NL2SQL_COLUMN WHERE question_id = :qid", qid=qid)
    # 유사질문 테이블은 없을 수 있음(질문관리 화면 미사용 DB) — ORA-00942 무시.
    try:
        await db.execute(database, "DELETE FROM T_NL2SQL_SIMILAR WHERE question_id = :qid", qid=qid)
    except Exception:  # noqa: BLE001
        pass
...
# ── 질문관리 화면 전용 CRUD (질문 수정 / 컬럼 수정·삭제 / 유사질문) ──────────
@router.put("/questions/{qid}")
async def update_question(qid: int, payload: dict, database: str = Depends(current_db)) -> dict:
    """질문 텍스트 수정. embed_params 있으면 question_vector 재생성(실패 시 텍스트만)."""
    question = (payload.get("question") or "").strip()
    embed_params = (payload.get("embed_params") or "").strip()
    if not question:
        raise HTTPException(status_code=400, detail={"error": "질문을 입력하세요"})
    await _ensure_admin_tables(database)
    embedded = await _q_update(database, qid, question, embed_params)
    warning = "embed_failed" if (embed_params and not embedded) else None
    return {"id": qid, "question": question, "warning": warning}


@router.put("/columns/{cid}")
async def update_column(cid: int, payload: dict, database: str = Depends(current_db)) -> dict:
    """조회컬럼명 수정."""
    await _ensure_tables(database)
    name = (payload.get("column_name") or "").strip()
    if not name:
        raise HTTPException(status_code=400, detail={"error": "컬럼명을 입력하세요"})
    await db.execute(
        database, "UPDATE T_NL2SQL_COLUMN SET column_name = :name WHERE id = :cid",
        name=name, cid=cid,
    )
    return {"id": cid, "column_name": name}


@router.delete("/columns/{cid}")
async def delete_column(cid: int, database: str = Depends(current_db)) -> dict:
    """조회컬럼 삭제."""
    await _ensure_tables(database)
    await db.execute(database, "DELETE FROM T_NL2SQL_COLUMN WHERE id = :cid", cid=cid)
    return {"ok": True}


@router.get("/questions/{qid}/similars")
async def list_similars(qid: int, database: str = Depends(current_db)) -> list[dict]:
    """질문에 연결된 유사질문(벡터 유무 포함)."""
    await _ensure_admin_tables(database)
    return await db.fetch_all(
        database,
        "SELECT id, similar_question, "
        "CASE WHEN similar_vector IS NULL THEN 0 ELSE 1 END AS has_vector "
        "FROM T_NL2SQL_SIMILAR WHERE question_id = :qid ORDER BY id",
        qid=qid,
    )


@router.post("/similars")
async def add_similar(payload: dict, database: str = Depends(current_db)) -> dict:
    """유사질문 등록(+embed_params 시 similar_vector 임베딩)."""
    qid = payload.get("question_id")
    text = (payload.get("similar_question") or "").strip()
    embed_params = (payload.get("embed_params") or "").strip()
    if qid is None:
        raise HTTPException(status_code=400, detail={"error": "대표 질문이 없습니다"})
    if not text:
        raise HTTPException(status_code=400, detail={"error": "유사질문을 입력하세요"})
    await _ensure_admin_tables(database)
    new_id, embedded = await _s_insert(database, qid, text, embed_params)
    warning = "embed_failed" if (embed_params and not embedded) else None
    return {"id": new_id, "similar_question": text, "question_id": qid,
            "has_vector": 1 if embedded else 0, "warning": warning}


@router.put("/similars/{sid}")
async def update_similar(sid: int, payload: dict, database: str = Depends(current_db)) -> dict:
    """유사질문 수정(+embed_params 시 벡터 재생성)."""
    text = (payload.get("similar_question") or "").strip()
    embed_params = (payload.get("embed_params") or "").strip()
    if not text:
        raise HTTPException(status_code=400, detail={"error": "유사질문을 입력하세요"})
    await _ensure_admin_tables(database)
    embedded = await _s_update(database, sid, text, embed_params)
    warning = "embed_failed" if (embed_params and not embedded) else None
    return {"id": sid, "similar_question": text,
            "has_vector": 1 if embedded else 0, "warning": warning}


@router.delete("/similars/{sid}")
async def delete_similar(sid: int, database: str = Depends(current_db)) -> dict:
    """유사질문 삭제."""
    await _ensure_admin_tables(database)
    await db.execute(database, "DELETE FROM T_NL2SQL_SIMILAR WHERE id = :sid", sid=sid)
    return {"ok": True}
